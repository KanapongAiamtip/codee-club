...
import { snakeize } from '@codee-club/common/dist/utils/string-cases'
...
const main = async (): Promise<void> => {
  ...
  fs.readdirSync(configPath).forEach((filename) => {
    ...
    if (!envObject.VUE_APP_EMU_PORT_STORAGE) envObject.VUE_APP_EMU_PORT_STORAGE = ''

    // Merge - firebase-config
    entries(firebaseConfig).forEach(([key, value]) => {
      envObject[VUE_FIREBASE_PREFIX + snakeize(key).toUpperCase()] = value
    })

    // Write new .env
    const updatedEnv = entries(envObject)
